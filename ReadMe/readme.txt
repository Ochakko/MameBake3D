...
開発環境(2018/02/10時点)を書きます。

Mac(ちくわ)とVMWare FusionとWindows10 Pro で開発しています。

上記以外の環境、
Boot Campを一瞬試してみましたが
GUIのクリックがうまくいきませんでした。
描画速度が速すぎると物理が揺れませんでした。
...
追記：
今後の開発環境ですが、
2018/06以降に発売されるという噂の
Mac Proのニューバージョンを狙っています
まだ決めていませんが。


##############################################
追記；Boot Camp環境でうまくいかない件への見解
##############################################
問題は２つあります。
１、メインウインドウのボタン類が押せない。
２、描画速度が速すぎると物理が揺れない。

１について
１はDXUT絡みの問題です。
現在の環境でも最初（DX9からDX10への移行時に）ボタン類が押せませんでした。
デバッグビルドで調べたところ、
指定したウインドウの大きさとバックバッファの大きさが異なるという部分が
assertしました。
これはアプリケーションにタイトルバーやメニューを付けることで
assertしたものと考えられます。
そこでDXUTの機能でOverrideWidth, OverrideHeightというのがあったので
これを利用することにしました。
OverrideWidth, OverrideHeightを指定してバックバッファを作成することで
assertで引っかからなくなりました。
更にチェックボックスのFocus処理をコメントアウトすることで
メインウインドウのボタン類が押せるようになりました。
BootCamp環境ではこの処理が必要ない、あるいは更なる対策が必要である可能性があります。


２について
描画速度が速すぎると物理が揺れないのは
利用しているBullet Physicsが60fpsを想定して設計されていることが
原因であると考えられます。
よって60fpsのfps上限処理を加えると解決すると思われます。
...
[その後の変更で物理シミュの開始と停止はメインウインドウのボタンで行います。
ラグドールは一時取りやめになり、修正して物理IK（PhysicsRotとPhysicsMV）になっています。]
...
０フレームはバンドポーズのために必要なので
今後の更新で再生を１フレームから始めるようにする予定です。

[2018/01/27にアニメーションの再生は１フレームから開始となりました。]
