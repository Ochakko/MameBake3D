...
{
	if( m_curre ){
		m_curre->m_groupid = m_groupid;
		m_curre->m_coliids = m_coliids;


		UINT ischecked;
		ischecked = m_dlg_wnd.IsDlgButtonChecked( IDC_MYSELF );
		if( ischecked == BST_CHECKED ){
			m_curre->m_myselfflag = 1;
		}else{
			m_curre->m_myselfflag = 0;
		}
...
{
	m_groupid = m_curre->m_groupid;
	m_coliids = m_curre->m_coliids;
	m_myself = m_curre->m_myselfflag;
