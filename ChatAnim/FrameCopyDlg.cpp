...
LRESULT CFrameCopyDlg::OnOK(WORD wNotifyCode, WORD wID, HWND hWndCtl, BOOL& bHandled)
{
	...
	int bonenum = m_model->m_bonelist.size();
...
int CFrameCopyDlg::FillTree()
{
	...
	int shdnum;
	shdnum = m_model->m_bonelist.size();

	m_timap.clear();

	m_selectedno = 0;
	m_selecteditem = 0;
	AddBoneToTree( m_model->m_topbone, 1, 1 );
...
void CFrameCopyDlg::AddBoneToTree( CBone* srcbone, int addbroflag, int addtolast )
{
	...
	if( parbone ){
		parTI = m_timap[ parbone->m_boneno ];
	}else{
		parTI = TreeView_GetRoot( m_tree_wnd );
	}

	HTREEITEM newTI;
	newTI = TVAdd( parTI, srcbone->m_wbonename, srcbone->m_boneno, m_iImage, m_iSelect, addtolast );
	if( !newTI ){
		_ASSERT( 0 );
		return;
	}

	m_timap[ srcbone->m_boneno ] = newTI;
	if( m_selectedno == 0 ){
		m_selecteditem = newTI;
		m_selectedno = srcbone->m_boneno;
		TreeView_SelectItem( m_tree_wnd, m_selecteditem );
	}

///////////
	if( srcbone->m_child ){
		AddBoneToTree( srcbone->m_child, 1, 1 );
	}

	///////////
	if( addbroflag ){
		if( srcbone->m_brother ){
			AddBoneToTree( srcbone->m_brother, 1, 1 );
		}
	}
...
int CFrameCopyDlg::ParamsToDlg()
{
	...
	for( listno = 0; listno < m_influencenum[m_slotno]; listno++ ){
		...
		WCHAR* bonename = m_model->m_bonelist[ m_influencelist[m_slotno][listno] ]->m_wbonename;
		WCHAR addstr[2048];
	...
	for( listno2 = 0; listno2 < m_ignorenum[m_slotno]; listno2++ ){
		...
		WCHAR* bonename = m_model->m_bonelist[ m_ignorelist[m_slotno][listno2] ]->m_wbonename;
		WCHAR addstr[2048];
		ZeroMemory( addstr, sizeof( WCHAR ) * 2048 );
...
LRESULT CFrameCopyDlg::OnAdd(WORD wNotifyCode, WORD wID, HWND hWndCtl, BOOL& bHandled)
{
	...
	_ASSERT( m_model->m_bonelist[ m_selectedno ] );
	WCHAR* bonename = m_model->m_bonelist[ m_selectedno ]->m_wbonename;
	WCHAR addstr[2048];
	ZeroMemory( addstr, sizeof( WCHAR ) * 2048 );
...
int CFrameCopyDlg::SetTree2ListReq( int validorinvalid, int srcno, int addbroflag )
{
	...
	if( (srcno >= 0) && (srcno < bonenum) ){
		...
		curbone = m_model->m_bonelist[ srcno ];
		if( curbone ){
			if( validorinvalid == 0 ){
	...
	if( curbone->m_child ){
		SetTree2ListReq( validorinvalid, curbone->m_child->m_boneno, 1 );
	}

	if( addbroflag == 1 ){
		if( curbone->m_brother ){
			SetTree2ListReq( validorinvalid, curbone->m_brother->m_boneno, 1 );
		}
	}
