...
/////////
	...
	s_btgscSlider->setCursorListener( [](){
		...
		if( s_model && (s_curreindex >= 0) ){
			s_model->m_rigideleminfo[ s_curreindex ].btgscale = btgsc;
		}
		s_rigidWnd->callRewrite();						//再描画
	} );
	...
	s_dmpB->setButtonListener( [](){ 
		if( s_model ){
			float ldmp = s_ldmpSlider->getValue();
			float admp = s_admpSlider->getValue();
			int chkg = (int)s_groupcheck->getValue();
			int gid = -1;
			if( chkg ){
				CRigidElem* curre = GetCurRe();
	...
	s_massB->setButtonListener( [](){ 
		if( s_model ){
			float mass = s_massSlider->getValue();
			int chkg = (int)s_groupcheck->getValue();
			int gid = -1;
			if( chkg ){
				CRigidElem* curre = GetCurRe();
...
//		_ASSERT( 0 );
	...
	s_btgB->setButtonListener( [](){ 
		if( s_model ){
			float btg = s_btgSlider->getValue();
			int chkg = (int)s_groupcheck->getValue();
			int gid = -1;
			if( chkg ){
				CRigidElem* curre = GetCurRe();
...
//	g_Camera.SetProjParams( D3DX_PI / 4, 1.0f, g_initnear, 4.0f * g_initcamdist );
	...
	CallF( s_select->LoadMQO( s_pdev, L"..\\Media\\PiyoMedia\\select_2.mqo", 0, 1.0f, 0 ), return 1 );
...
//		return 1;
//	}
//	CallF( s_dummytri->LoadMQO( s_pdev, L"..\\Media\\PiyoMedia\\dummytri.mqo", 0, 1.0f, 1 ), return 1 );
...
//	CallF( s_dummytri->MakeDispObj(), return 1 );
	...
	CallF( s_bmark->LoadMQO( s_pdev, L"..\\Media\\PiyoMedia\\bonemark.mqo", 0, 1.0f, 0 ), return 1 );
	...
	CallF( s_coldisp[COL_CONE_INDEX]->LoadMQO( s_pdev, L"..\\Media\\PiyoMedia\\cone_dirY.mqo", 0, 1.0f, 0 ), return 1 );
	...
	CallF( s_coldisp[COL_CAPSULE_INDEX]->LoadMQO( s_pdev, L"..\\Media\\PiyoMedia\\capsule_dirY.mqo", 0, 1.0f, 0 ), return 1 );
	...
	CallF( s_coldisp[COL_SPHERE_INDEX]->LoadMQO( s_pdev, L"..\\Media\\PiyoMedia\\sphere_dirY.mqo", 0, 1.0f, 0 ), return 1 );
	...
	CallF( s_coldisp[COL_BOX_INDEX]->LoadMQO( s_pdev, L"..\\Media\\PiyoMedia\\box.mqo", 0, 1.0f, 0 ), return 1 );
	...
	CallF( s_ground->LoadMQO( s_pdev, L"..\\Media\\PiyoMedia\\ground2.mqo", 0, 1.0f, 0 ), return 1 );
	...
	CallF( s_gplane->LoadMQO( s_pdev, L"..\\Media\\PiyoMedia\\gplane.mqo", 0, 1.0f, 0, D3DPOOL_MANAGED ), return 1 );
	...
	*last2en = 0L;
	wcscat_s( path, MAX_PATH, L"\\Media\\PiyoMedia\\" );
...
///////
	...
	*last2en = 0L;
	wcscat_s( mpath, MAX_PATH, L"\\Media\\PiyoMedia\\" );
...
//				_ASSERT( 0 );
	...
	if( s_cursorFlag ){
		...
		// カーソル位置をコンソールに出力
		if( s_owpTimeline && s_model && s_model->m_curmotinfo ){
...
***/		
	...
	if( s_LcursorFlag ){
		s_LcursorFlag = false;
		if( s_owpLTimeline && s_model && s_model->m_curmotinfo ){
	...
	if( s_initmpFlag ){
		...
		if( s_model && s_owpTimeline && s_owpLTimeline && s_model->m_curmotinfo){
			s_copymotmap.clear();
				...
				if( cpnum != 0 ){
					...
					for( cpno = 0; cpno < cpnum; cpno++ ){
						...
						if( curbone ){
							...
							CreateTimeLineMark( curbone->m_boneno );
						}
					}
					for( cpno = 0; cpno < cpnum; cpno++ ){
						CBone* curbone = s_selbonedlg.m_cpvec[ cpno ];
	...
	if( s_pasteFlag ){
		...
		if( s_model && s_owpTimeline && s_model->m_curmotinfo && !s_copymotmap.empty() ){
			...
			for( itrcp = s_copymotmap.begin(); itrcp != s_copymotmap.end(); itrcp++ ){
				if( itrcp->second.m_frame <= copyStartTime ){
					copyStartTime = itrcp->second.m_frame;
				}
			}
			...
			for( itrcp = s_copymotmap.begin(); itrcp != s_copymotmap.end(); itrcp++ ){
				...
				double newframe = srcmp.m_frame - copyStartTime + s_owpLTimeline->getCurrentTime();
				int existflag2 = 0;
				CMotionPoint* pbef = 0;
				CMotionPoint* pnext = 0;
				int curmotid = s_model->m_curmotinfo->motid;
				srcbone->GetBefNextMP( curmotid, newframe, &pbef, &pnext, &existflag2 );
				if( existflag2 ){
					newmp = pbef;
					if( cptopflag == 1 ){
						newmp->m_befworldmat = newmp->m_worldmat;
					}
					newmp->m_worldmat = srcmp.m_worldmat;
				}
			}
			...
			for( itrcp = s_copymotmap.begin(); itrcp != s_copymotmap.end(); itrcp++ ){
				...
				if( existflag2 ){
					...
					if( cptopflag == 1 ){
						D3DXVECTOR3 orgpos;
						D3DXVec3TransformCoord( &orgpos, &(srcbone->m_jointfpos), &(newmp->m_befworldmat) );

						D3DXVECTOR3 newpos;
						D3DXVec3TransformCoord( &newpos, &(srcbone->m_jointfpos), &(newmp->m_worldmat) );
						...
						s_model->FKBoneTra( &tmper, srcbone->m_boneno, curmotid, newframe, diffpos );
					}
				}
			}
		}
...
///////////// undo
	if( s_model && g_controlkey && (g_keybuf[ 'Z' ] & 0x80) && !(g_savekeybuf[ 'Z' ] & 0x80) ){
		...
		if( s_model->m_curmotinfo->motid != s_curmotid ){
			int chkcnt = 0;
			int findflag = 0;
...
//	mW._41, mW._42, mW._43, mW._44 );
	...
	if( s_gplane && s_bpWorld && s_bpWorld->m_rigidbodyG ){
		...
		s_gplane->UpdateMatrix( -1, &gpmat, &mVP );
	}

	if( s_model && (s_model->m_btcnt == 0) ){
		if( !s_bpWorld ){
...
int InsertSymMP( CBone* curbone, double curframe )
{
	int symboneno = 0;
	int existflag = 0;
	s_model->GetSymBoneNo( curbone->m_boneno, &symboneno, &existflag );
	if( symboneno >= 0 ){
		CBone* symbone = s_model->m_bonelist[ symboneno ];
		_ASSERT( symbone );
		int existflagsym = 0;
		CMotionPoint* pbefsym = 0;
		CMotionPoint* pnextsym = 0;
		symbone->GetBefNextMP( s_model->m_curmotinfo->motid, curframe, &pbefsym, &pnextsym, &existflagsym );
		_ASSERT( existflagsym && pbefsym );

		CQuaternion symq;
		symq.SetParams( 1.0f, 0.0f, 0.0f, 0.0f );
		D3DXMATRIX symmat0 = symq.CalcSymX( pbefsym->m_worldmat );

		D3DXVECTOR3 sympos;
		D3DXVec3TransformCoord( &sympos, &(symbone->m_jointfpos), &(pbefsym->m_worldmat) );
		sympos.x *= -1.0f;

		D3DXVECTOR3 orgpos;
		D3DXVec3TransformCoord( &orgpos, &(curbone->m_jointfpos), &symmat0 );
		...
		CMotionPoint addmp;
		addmp.m_frame = curframe;
		addmp.m_worldmat = symmat;
...
int InitMP( CBone* curbone, double curframe )
{
	...
	if( curbone->m_parent ){
		curbone->m_parent->GetBefNextMP( s_model->m_curmotinfo->motid, curframe, &pbef1, &pnext1, &existflag1 );
	}

	if( existflag && pbef ){

		pbef->m_befworldmat = pbef->m_worldmat;

		if( curbone->m_child ){
			WCHAR bunkipat[256] = L"_bunki";
			WCHAR* findpat = wcsstr( curbone->m_wbonename, bunkipat );
			if( !findpat ){
				D3DXMatrixIdentity( &(pbef->m_worldmat) );
			}else{
				if( existflag1 && pbef1 ){
					D3DXMATRIX invfirst;
					D3DXMatrixInverse( &invfirst, NULL, &(curbone->m_parent->m_firstmat) );
					pbef->m_worldmat = invfirst;
				}else{
					D3DXMatrixIdentity( &(pbef->m_worldmat) );
				}
			}
...
int AdjustBoneTra( CBone* curbone, double curframe )
{
	...
	if( existflag && pbef ){
		D3DXVECTOR3 orgpos;
		D3DXVec3TransformCoord( &orgpos, &(curbone->m_jointfpos), &(pbef->m_befworldmat) );

		D3DXVECTOR3 newpos;
		D3DXVec3TransformCoord( &newpos, &(curbone->m_jointfpos), &(pbef->m_worldmat) );
		...
		s_model->FKBoneTra( &tmper, curbone->m_boneno, s_model->m_curmotinfo->motid, curframe, diffpos );
	}
...
void CALLBACK OnFrameRender( IDirect3DDevice9* pd3dDevice, double fTime, float fElapsedTime, void* pUserContext )
{
    ...
    if( SUCCEEDED( pd3dDevice->BeginScene() ) )
    {
		...
		if( (g_previewFlag != 4) && (g_previewFlag != 5) ){
			if( s_model && s_model->m_modeldisp ){
				if( s_ikkind >= 3 ){
		...
		if( (g_previewFlag != 4) && (g_previewFlag != 5) ){
			if( s_select && (s_curboneno >= 0) && (g_previewFlag == 0) && (s_model && s_model->m_modeldisp) ){
				//SetSelectCol();
				SetSelectState();
				RenderSelectMark();
			}
...
//s_model->CalcBtAxismat( delta );//!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
		...
		if( s_model && (s_curboneno >= 0) && s_camtargetflag ){
			CBone* curbone = s_model->m_bonelist[ s_curboneno ];
			_ASSERT( curbone );
			g_camtargetpos = curbone->m_childworld;
		}
...
void CALLBACK OnGUIEvent( UINT nEvent, int nControlID, CDXUTControl* pControl, void* pUserContext )
{
    ...
    switch( nControlID )
    {
        ...
        case IDC_SL_APPLYRATE:
            g_applyrate = ( float )( g_SampleUI.GetSlider( IDC_SL_APPLYRATE )->GetValue() );
		    swprintf_s( sz, 100, L"姿勢適用位置 : %d ％", g_applyrate );
...
int OpenImpFile()
{
	...
	int impnum = s_model->m_impinfo.size();
	if( impnum > 0 ){
		OnImpMenu( impnum - 1 );
	}else{
		OnImpMenu( -1 );
...
int OpenREFile()
{
	...
	if( s_model->m_rigideleminfo.size() >= (MAXRENUM - 1) ){
	...
	int renum = s_model->m_rigideleminfo.size();
	if( renum > 0 ){
		OnREMenu( renum - 1, 0 );
		OnRgdMenu( renum - 1, 0 );
	}else{
...
int SaveREFile()
{
	...
	CRigidElemFile refile;
	CallF( refile.WriteRigidElemFile( s_REname, s_model, s_model->m_btgscale ), return 1 );
...
//	SetCapture( s_mainwnd );
	...
	s_curmotid = s_model->m_curmotinfo->motid;
...
void refreshTimeline(OWP_Timeline& timeline){
	...
	if( s_model && (s_model->m_curmotinfo) ){
		...
		s_owpLTimeline->setMaxTime( s_model->m_curmotinfo->frameleng );//左端の１マスを選んだ状態がフレーム０を選んだ状態だから　-1 しない。

		int itime;
		for( itime = 0; itime < (int)s_model->m_curmotinfo->frameleng; itime++ ){
...
int AddBoneEul( int kind, float adddeg )
{
	...
	D3DXMATRIX worldrot = curbone->m_axismat_par * curbone->m_curmp.m_worldmat;
	worldrot._41 = 0.0f;
	worldrot._42 = 0.0f;
	worldrot._43 = 0.0f;
	...
	D3DXMATRIX rotmat;
	D3DXMatrixRotationAxis( &rotmat, &basevec, adddeg * DEG2PAI );
	...
	int curmotid = s_model->m_curmotinfo->motid;
	double curframe = s_owpTimeline->getCurrentTime();
	s_model->FKRotate( &s_editrange, s_curboneno, curmotid, curframe, rotq );
...
{
	if( !s_model || (s_curboneno < 0) && !s_model->m_topbone ){
		return 0;
	}
	...
	int curmotid = s_model->m_curmotinfo->motid;
	double curframe = s_owpTimeline->getCurrentTime();
	s_model->FKBoneTra( &s_editrange, s_curboneno, curmotid, curframe, diffvec );
...
int AddBoneTra( int kind, float srctra )
{
	...
	int curmotid = s_model->m_curmotinfo->motid;
	double curframe = s_owpTimeline->getCurrentTime();
	s_model->FKBoneTra( &s_editrange, s_curboneno, curmotid, curframe, addtra );
...
int OnModelMenu( int selindex, int callbymenu )
{
	...
	if( s_model ){
		if( s_model->m_rigideleminfo.size() > 0 ){
...
int OnREMenu( int selindex, int callbymenu )
{
	...
	cReSets = s_model->m_rigideleminfo.size();
	if( cReSets <= 0 ){
		s_curreindex = -1;
		AppendMenu(s_remenu, MF_STRING, 62000, L"読み込んでいません" );
		return 0;//!!!!!!!!!!!!!!!!!!!
	...
	for(iReSet = 0; iReSet < cReSets; iReSet++)
	{
		...
		MultiByteToWideChar( CP_ACP, MB_PRECOMPOSED, s_model->m_rigideleminfo[ iReSet ].filename, MAX_PATH, wname, MAX_PATH );
...
{
	if( s_model ){
		s_model->m_curimpindex = selindex;
	}
	...
	cReSets = s_model->m_impinfo.size();
	if( cReSets <= 0 ){
		s_model->m_curimpindex = 0;
		AppendMenu(s_impmenu, MF_STRING, 64500, L"読み込んでいません" );
		return 0;//!!!!!!!!!!!!!!!!!!!
	}
	...
	{
		ZeroMemory( wname, sizeof( WCHAR ) * MAX_PATH );
		MultiByteToWideChar( CP_ACP, MB_PRECOMPOSED, s_model->m_impinfo[ iReSet ].c_str(), -1, wname, MAX_PATH );
...
int RenderSelectMark()
{
	...
	if( curboneptr ){
		D3DXMATRIX bonetra;
		s_selm = curboneptr->m_axismat_par * curboneptr->m_curmp.m_worldmat;

		D3DXVECTOR3 orgpos = curboneptr->m_jointfpos;
...
int OnAddMotion( int srcmotid )
{
	...
	MOTINFO* newmotinfo = s_model->m_motinfo[ srcmotid ];
...
int RigidElem2WndParam()
{
	...
	if( curbone ){
		...
		CBone* parbone = curbone->m_parent;
		if( parbone ){
			int kinflag = parbone->m_btforce;
			s_btforce->setValue( (bool)kinflag );
			
			CRigidElem* curre = parbone->m_rigidelem[curbone];
			if( curre ){
				float rate = (float)curre->m_shprate;
				float boxz = (float)curre->m_boxzrate;
				float mass = (float)curre->m_mass;
				bool skipflag = !((bool)curre->m_skipflag);
				bool bcone = curre->m_bcone;
				int colindex = curre->m_coltype;
				int lkindex = curre->m_l_kindex;
				int akindex = curre->m_a_kindex;
				float ldmp = curre->m_l_damping;
				float admp = curre->m_a_damping;
				float cuslk = curre->m_cus_lk;
				float cusak = curre->m_cus_ak;
				float rest = curre->m_restitution;
				float fric = curre->m_friction;
				float btg = curre->m_btg;
		...
		s_namelabel->setName( curbone->m_wbonename );
	}else{
		WCHAR noname[256];
	...
	if( s_model ){
		s_btgscSlider->setValue( s_model->m_btgscale );
	}
...
int SetRigidLeng()
{
	...
	if( curbto ){
		WCHAR curlabel[512];
		swprintf_s( curlabel, 512, L"ボーン名：%s", curbto->m_endbone->m_wbonename );
		if( s_namelabel ){
			s_namelabel->setName( curlabel );
		}

		WCHAR curlabel2[256];
		swprintf_s( curlabel2, 256, L"ボーン長:%.3f[m]", curbto->m_boneleng );
		if( s_lenglabel ){
...
int SetImpWndParams()
{
	...
	if( curbone ){
		...
		if( parbone ){
			...
			if( (s_model->m_curimpindex >= 0) && (s_model->m_curimpindex < impnum) ){

				string curimpname = s_model->m_impinfo[ s_model->m_curimpindex ];
...
int SetDmpWndParams()
{
	...
	if( curbone ){
		CBone* parbone = curbone->m_parent;
		if( parbone ){
			char* filename = s_model->m_rigideleminfo[s_rgdindex].filename;
			CRigidElem* curre = (parbone->m_remap[ filename ])[ curbone ];
			if( curre ){
				if( s_dmpanimLSlider ){
					s_dmpanimLSlider->setValue( curre->m_dampanim_l );
				}
				if( s_dmpanimASlider ){
					s_dmpanimASlider->setValue( curre->m_dampanim_a );
				}
			}
		}
	}
...
int SaveProject()
{
	...
	CChaFile chafile;
	CallF( chafile.WriteChaFile( s_bpWorld, s_projectdir, s_projectname, s_modelindex, g_dspeed ), return 1 );
...
int OnSetMotSpeed()
{
	s_model->m_curmotinfo->speed = s_model->m_tmpmotspeed;//!!!!!!!!!!!!!!!!!!!
	g_dspeed = s_model->m_tmpmotspeed;
...
CRigidElem* GetCurRe()
{
	...
	if( s_model && (s_curboneno >= 0) && (s_curreindex >= 0) ){
		...
		if( curbone ){
			CBone* parbone = curbone->m_parent;
			if( parbone ){
				char* filename = s_model->m_rigideleminfo[s_curreindex].filename;
				CRigidElem* curre = (parbone->m_remap[ filename ])[ curbone ];
				if( curre ){
					retre = curre;
				}else{
...
//	s_owpTimeline->deleteKey();
	...
	for( itrbone = s_model->m_bonelist.begin(); itrbone != s_model->m_bonelist.end(); itrbone++ ){
		CBone* curbone = itrbone->second;
		if( curbone ){
			int curlineno = s_boneno2lineno[ curbone->m_boneno ];
			if( curlineno >= 0 ){
				double curframe;
				for( curframe = 0.0; curframe < s_model->m_curmotinfo->frameleng; curframe += 1.0 ){
	...
	for( itrbone = s_model->m_bonelist.begin(); itrbone != s_model->m_bonelist.end(); itrbone++ ){
		...
		if( curbone ){
			...
			if( curlineno >= 0 ){
				...
				itrcur = curbone->m_motmark.find( s_model->m_curmotinfo->motid );
				if( itrcur != curbone->m_motmark.end() ){
					map<double, int> curmark;
					curmark = itrcur->second;

					map<double, int>::iterator itrmark;
					for( itrmark = curmark.begin(); itrmark != curmark.end(); itrmark++ ){
						double curframe = itrmark->first;
						//KeyInfo chkki = s_owpTimeline->ExistKey( curlineno, curframe );
						//if( chkki.lineIndex < 0 ){
							s_owpTimeline->newKey( curbone->m_wbonename, curframe, 0 );
						//}
					}
				}
			}
...
void CreateMarkReq( int curboneno, int broflag )
{
	...
	if( curbone->m_child ){
		CreateMarkReq( curbone->m_child->m_boneno, 1 );
	}
	if( broflag && curbone->m_brother ){
		CreateMarkReq( curbone->m_brother->m_boneno, 1 );
	}
}
	...
	if( (curboneno >= 0) && s_model && s_owpTimeline && s_owpLTimeline ){
		...
		if( curbone ){
			...
			if( curlineno >= 0 ){
				...
				swprintf_s( markname, 256, L"Mark:%s", curbone->m_wbonename );
				s_owpLTimeline->newLine( 0, markname );

				double frame;
				for( frame = 0.0; frame < s_model->m_curmotinfo->frameleng; frame += 1.0 ){
