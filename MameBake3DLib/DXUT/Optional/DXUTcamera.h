...
public:
    ...
    const ChaMatrix* GetRotationMatrix()                   { return ChaMatrixRotationQuaternion(&m_mRotation, &m_qNow); };
    const ChaMatrix* GetTranslationMatrix() const          { return &m_mTranslation; }
    const ChaMatrix* GetTranslationDeltaMatrix() const     { return &m_mTranslationDelta; }
    bool        IsBeingDragged() const                      { return m_bDrag; }
    CQuaternion GetQuatNow() const                       { return m_qNow; }
    void        SetQuatNow( CQuaternion q ) { m_qNow = q; }

    static CQuaternion WINAPI    QuatFromBallPoints( const ChaVector3& vFrom, const ChaVector3& vTo );


protected:
    ChaMatrix m_mRotation;         // Matrix for arc ball's orientation
    ChaMatrix m_mTranslation;      // Matrix for arc ball's position
    ChaMatrix m_mTranslationDelta; // Matrix for arc ball's position
    ...
    CQuaternion m_qDown;             // Quaternion before button down
    CQuaternion m_qNow;              // Composite quaternion for current drag
    bool m_bDrag;             // Whether user is dragging arc ball

    POINT m_ptLastMouse;      // position of last mouse point
    ChaVector3 m_vDownPt;           // starting point of rotation arc
    ChaVector3 m_vCurrentPt;        // current point of rotation arc

    ChaVector3                     ScreenToVector( float fScreenPtX, float fScreenPtY );
};
...
public:
    ...
    virtual void                Reset();
    virtual void                SetViewParams( ChaVector3* pvEyePt, ChaVector3* pvLookatPt );
    ...
    void SetClipToBoundary( bool bClipToBoundary, ChaVector3* pvMinBoundary, ChaVector3* pvMaxBoundary ) { m_bClipToBoundary = bClipToBoundary; if( pvMinBoundary ) m_vMinBoundary = *pvMinBoundary; if( pvMaxBoundary ) m_vMaxBoundary = *pvMaxBoundary; }
    ...
    const ChaMatrix*  GetViewMatrix() const { return &m_mView; }
    const ChaMatrix*  GetProjMatrix() const { return &m_mProj; }
    const ChaVector3* GetEyePt() const      { return &m_vEye; }
    const ChaVector3* GetLookAtPt() const   { return &m_vLookAt; }
...
protected:
    ...
    ChaVector3 m_vKeyboardDirection;   // Direction vector of keyboard input
    ...
    ChaVector3 m_vDefaultEye;          // Default camera eye position
    ChaVector3 m_vDefaultLookAt;       // Default LookAt position
    ChaVector3 m_vEye;                 // Camera eye position
    ChaVector3 m_vLookAt;              // LookAt position
    ...
    ChaVector3 m_vMinBoundary;         // Min point in clip boundary
    ChaVector3 m_vMaxBoundary;         // Max point in clip boundary
...
public:
    ...
    // Functions to get state
    ChaMatrix*  GetWorldMatrix()            { return &m_mCameraWorld; }

    const ChaVector3* GetWorldRight() const { return (ChaVector3*)&m_mCameraWorld._11; } 
    const ChaVector3* GetWorldUp() const    { return (ChaVector3*)&m_mCameraWorld._21; }
    const ChaVector3* GetWorldAhead() const { return (ChaVector3*)&m_mCameraWorld._31; }
    const ChaVector3* GetEyePt() const      { return (ChaVector3*)&m_mCameraWorld._41; }

protected:
    ChaMatrix m_mCameraWorld;       // World matrix of the camera (inverse of the view matrix)
...
public:
    ...
    void            Reset();
    void            SetViewParams( ChaVector3* pvEyePt, ChaVector3* pvLookatPt );
    ...
    void SetModelCenter( ChaVector3 vModelCenter ) { m_vModelCenter = vModelCenter; }
    void SetLimitPitch( bool bLimitPitch ) { m_bLimitPitch = bLimitPitch; }
    void SetViewQuat( CQuaternion q ) { m_ViewArcBall.SetQuatNow( q ); m_bDragSinceLastUpdate = true; }
    void SetWorldQuat( CQuaternion q ) { m_WorldArcBall.SetQuatNow( q ); m_bDragSinceLastUpdate = true; }

    // Functions to get state
    const ChaMatrix* GetWorldMatrix() const { return &m_mWorld; }
    void SetWorldMatrix( ChaMatrix &mWorld ) { m_mWorld = mWorld; m_bDragSinceLastUpdate = true; }

protected:
    CD3DArcBall m_WorldArcBall;
    CD3DArcBall m_ViewArcBall;
    ChaVector3 m_vModelCenter;
    ChaMatrix m_mModelLastRot;        // Last arcball rotation matrix for model 
    ChaMatrix m_mModelRot;            // Rotation matrix of model
    ChaMatrix m_mWorld;               // World matrix of model
...
public:
    ...
    HRESULT OnRender9( D3DXCOLOR color, const ChaMatrix* pmView, const ChaMatrix* pmProj, const ChaVector3* pEyePt );
    LRESULT                 HandleMessages( HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam );
    ...
    HRESULT OnRender10( D3DXCOLOR color, const ChaMatrix* pmView, const ChaMatrix* pmProj, const ChaVector3* pEyePt );
    static void WINAPI      StaticOnD3D10DestroyDevice();

    ChaVector3 GetLightDirection()         { return m_vCurrentDir; };
    void        SetLightDirection( ChaVector3 vDir ) { m_vDefaultDir = m_vCurrentDir = vDir; };
...
protected:
    ...
    ChaMatrix m_mRot;
    ChaMatrix m_mRotSnapshot;
    float m_fRadius;
    int m_nRotateMask;
    CD3DArcBall m_ArcBall;
    ChaVector3 m_vDefaultDir;
    ChaVector3 m_vCurrentDir;
    ChaMatrix m_mView;
};
