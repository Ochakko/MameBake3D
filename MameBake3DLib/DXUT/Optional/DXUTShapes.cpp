                          ...
                          float fRadius2, float fLength, UINT uSlices, UINT uStacks )
{
    ...
    pVertex->pos = ChaVector3( 0.0f, 0.0f, fZ );
    pVertex->norm = ChaVector3( 0.0f, 0.0f, -1.0f );
    pVertex++;
    if( pPointReps != NULL )
    ...
    for( i = 0; i < uSlices; i++ )
    {
        pVertex->pos = ChaVector3( fRadius * sinI[i], fRadius * cosI[i], fZ );
        pVertex->norm = ChaVector3( 0.0f, 0.0f, -1.0f );
        pVertex++;
    ...
    for( j = 0; j <= uStacks; j++ )
    {
        ...
        {
            pVertex->pos = ChaVector3( fRadius * sinI[i], fRadius * cosI[i], fZ );
            pVertex->norm = ChaVector3( fNormalXY * sinI[i], fNormalXY * cosI[i], fNormalZ );
            pVertex++;
            if( pPointReps != NULL )
                pPointReps[iVertex] = iVertex;
            iVertex++;
...
static void MakePolygon( VERTEX* pVertices, WORD* pwIndices, float fLength, UINT uSides )
{
    ...
    pVertex->pos = ChaVector3( 0.0f, 0.0f, 0.0f );
    pVertex->norm = ChaVector3( 0.0f, 0.0f, 1.0f );
    pVertex++;
    ...
    for( UINT j = 0; j < uSides; j++ )
    {
        ...
        pVertex->pos = ChaVector3( c * radius, s * radius, 0.0f );
        pVertex->norm = ChaVector3( 0.0f, 0.0f, 1.0f );
        pVertex++;
    }
...
static void MakeSphere( VERTEX* pVertices, WORD* pwIndices, float fRadius, UINT uSlices, UINT uStacks )
{
    ...
    // +Z pole
    pVertex->pos = ChaVector3( 0.0f, 0.0f, fRadius );
    pVertex->norm = ChaVector3( 0.0f, 0.0f, 1.0f );
    pVertex++;
    ...
    for( j = 1; j < uStacks; j++ )
    {
        for( i = 0; i < uSlices; i++ )
        {
            ChaVector3 norm( sinI[i]* sinJ[j], cosI[i]* sinJ[j], cosJ[j] );
...
static ChaVector3* teapotPositions = ( ChaVector3* )teapotPositionsFloats;
...
static ChaVector3* teapotNormals = ( ChaVector3* )teapotNormalsfloats;
