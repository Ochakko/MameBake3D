                                        ...
                                        SDKMESH_CALLBACKS9* pLoaderCallbacks9 )
{
    ...
    m_pBindPoseFrameMatrices = new ChaMatrix[ m_pMeshHeader->NumFrames ];
    if( !m_pBindPoseFrameMatrices )
        goto Error;

    // Create a place to store our transformed frame matrices
    m_pTransformedFrameMatrices = new ChaMatrix[ m_pMeshHeader->NumFrames ];
    ...
    for (UINT meshi=0; meshi < m_pMeshHeader->NumMeshes; ++meshi) {
        ...
        //BYTE* pIndices = NULL;
            ...
            for (UINT vertind = IndexStart; vertind < IndexStart + IndexCount; ++vertind) {
                ...
                tris++;
                ChaVector3 *pt = (ChaVector3*)&(verts[stride * current_ind]);
                if (pt->x < lower.x) {
                    lower.x = pt->x;
                }
                if (pt->y < lower.y) {
...
//--------------------------------------------------------------------------------------
void CDXUTSDKMesh::TransformBindPoseFrame( UINT iFrame, ChaMatrix* pParentWorld )
{
    if( !m_pBindPoseFrameMatrices )
        return;

    // Transform ourselves
    ChaMatrix LocalWorld;
    //ChaMatrixMultiply( &LocalWorld, &m_pFrameArray[iFrame].Matrix, pParentWorld );
	LocalWorld = m_pFrameArray[iFrame].Matrix * pParentWorld;
...
//--------------------------------------------------------------------------------------
void CDXUTSDKMesh::TransformFrame( UINT iFrame, ChaMatrix* pParentWorld, double fTime )
{
    // Get the tick data
    ChaMatrix LocalTransform;
    ...
    if( INVALID_ANIMATION_DATA != m_pFrameArray[iFrame].AnimationDataIndex )
    {
        ...
        ChaVector3 parentPos = pData->Translation;
        ChaMatrix mTranslate;
        ChaMatrixTranslation( &mTranslate, parentPos.x, parentPos.y, parentPos.z );
        ...
        if( quat.w == 0 && quat.x == 0 && quat.y == 0 && quat.z == 0 )
            CQuaternionIdentity( &quat );
        CQuaternionNormalize( &quat, &quat );
        ChaMatrixRotationQuaternion( &mQuat, &quat );
    ...
    // Transform ourselves
    ChaMatrix LocalWorld;
    //ChaMatrixMultiply( &LocalWorld, &LocalTransform, pParentWorld );
	LocalWorld = LocalTransform * *pParentWorld;
...
void CDXUTSDKMesh::TransformFrameAbsolute( UINT iFrame, double fTime )
{
    ...
    if( INVALID_ANIMATION_DATA != m_pFrameArray[iFrame].AnimationDataIndex )
    {
        ...
        ChaMatrixTranslation( &mTrans1, -pDataOrig->Translation.x,
                               -pDataOrig->Translation.y,
                               -pDataOrig->Translation.z );
        ChaMatrixTranslation( &mTrans2, pData->Translation.x,
                               pData->Translation.y,
                               pData->Translation.z );
        ...
        CQuaternionInverse( &quat1, &quat1 );
        ChaMatrixRotationQuaternion( &mRot1, &quat1 );
        ...
        ChaMatrixRotationQuaternion( &mRot2, &quat2 );
        mFrom = mRot2 * mTrans2;

        ChaMatrix mOutput = mInvTo * mFrom;
...
//--------------------------------------------------------------------------------------
void CDXUTSDKMesh::TransformBindPose( ChaMatrix* pWorld )
{
...
//--------------------------------------------------------------------------------------
void CDXUTSDKMesh::TransformMesh( ChaMatrix* pWorld, double fTime )
{
    ...
    if( FTT_RELATIVE == m_pAnimationHeader->FrameTransformType )
    {
        ...
        ChaMatrix mInvBindPose;
        ChaMatrix mFinal;
        for( UINT i = 0; i < m_pMeshHeader->NumFrames; i++ )
        {
            ChaMatrixInverse( &mInvBindPose, NULL, &m_pBindPoseFrameMatrices[i] );
...
//--------------------------------------------------------------------------------------
ChaVector3 CDXUTSDKMesh::GetMeshBBoxCenter( UINT iMesh )
{
    return m_pMeshArray[iMesh].BoundingBoxCenter;
}

//--------------------------------------------------------------------------------------
ChaVector3 CDXUTSDKMesh::GetMeshBBoxExtents( UINT iMesh )
{
...
//--------------------------------------------------------------------------------------
const ChaMatrix* CDXUTSDKMesh::GetMeshInfluenceMatrix( UINT iMesh, UINT iInfluence )
{
