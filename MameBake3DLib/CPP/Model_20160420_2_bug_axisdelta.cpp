...
int CModel::IKRotateAxisDelta(CEditRange* erptr, int axiskind, int srcboneno, float delta, int maxlevel, int ikcnt)
{
	...
	for (calccnt = 0; calccnt < calcnum; calccnt++){
		...
		while (curbone && ((maxlevel == 0) || (levelcnt < maxlevel))){
			...
			/*
			CQuaternion baseq;
			CQuaternion invbaseq;
			{
				CMotionPoint* pbef = 0;
				CMotionPoint* pnext = 0;
				int existflag = 0;
				curbone->GetBefNextMP(m_curmotinfo->motid, applyframe, &pbef, &pnext, &existflag);
				if (existflag && pbef){
					CMotionPoint basemp;
					basemp.CalcQandTra(pbef->GetWorldMat(), curbone);
					baseq = basemp.GetQ();
					baseq.inv(&invbaseq);
				}
				else{
					baseq.SetParams(1.0f, 0.0f, 0.0f, 0.0f);
					invbaseq.SetParams(1.0f, 0.0f, 0.0f, 0.0f);
				}
			}
			*/
			ChaMatrix selectmat;
			...
			CMotionPoint selmp;
			selmp.CalcQandTra(selectmat, parbone);
			CQuaternion selq = selmp.GetQ();
			CQuaternion invselq = selmp.GetInvQ();
			CMotionPoint oldmp;
			oldmp.CalcQandTra(firstbone->GetCurMp().GetWorldMat(), firstbone);
			CQuaternion oldq = oldmp.GetQ();
			CQuaternion involdq = oldmp.GetInvQ();
			CQuaternion rotq;
			//rotq = invselq * localq * oldq;
			//rotq = invbaseq * selq * localq * invselq * baseq;
			//rotq = involdq * invselq * localq * selq * oldq;
			//rotq = involdq * localq * selq;
			//rotq = involdq * selq * localq;
			rotq = involdq * selq * localq;
