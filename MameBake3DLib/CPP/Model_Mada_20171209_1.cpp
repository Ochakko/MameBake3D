...
//// 新しい回転を求める　ここから
							...
							//ChaMatrix curlocalworldrotmat;
							//curlocalworldrotmat = ChaMatrixFromBtMat3x3(curworldtra.getBasis()) * ChaMatrixFromBtMat3x3(parworldtra.getBasis().inverse());

							//btMatrix3x3 diffmat = firstworldmat.inverse() * curworldtra.getBasis();
							//btMatrix3x3 diffmat = curworldtra.getBasis() * firstworldmat.inverse();
							//btMatrix3x3 eulmat = contraA.getBasis().inverse() * diffmat * contraA.getBasis();
							//btMatrix3x3 eulmat = contraA.getBasis() * diffmat * contraA.getBasis().inverse();

							//ChaMatrix diffmat = ChaMatrixFromBtMat3x3(firstworldmat.inverse()) * ChaMatrixFromBtMat3x3(curworldtra.getBasis()) * ChaMatrixFromBtMat3x3(parworldtra.getBasis().inverse());
							//ChaMatrix diffmat = ChaMatrixFromBtMat3x3(curworldtra.getBasis()) * ChaMatrixFromBtMat3x3(parworldtra.getBasis().inverse());
							//ChaMatrix eulmat = ChaMatrixFromBtMat3x3(contraA.getBasis()) * diffmat * ChaMatrixFromBtMat3x3(contraA.getBasis().inverse());
							//ChaMatrix eulmat = ChaMatrixFromBtMat3x3(contraA.getBasis()) * ChaMatrixFromBtMat3x3(curworldtra.getBasis()) * ChaMatrixFromBtMat3x3(contraA.getBasis().inverse()) * ChaMatrixFromBtMat3x3(contraA.getBasis().inverse());
							//ChaMatrix eulmat = ChaMatrixFromBtMat3x3(contraA.getBasis().inverse()) * diffmat * ChaMatrixFromBtMat3x3(contraA.getBasis());
							//btMatrix3x3 diffmat = curworldtra.getBasis() * firstworldmat.inverse();
							//btMatrix3x3 eulmat = contraA.getBasis().inverse() * diffmat * contraA.getBasis();
							//btMatrix3x3 eulmat = contraA.getBasis() * diffmat * contraA.getBasis().inverse();
							//ChaMatrix eulmat = ChaMatrixFromBtMat3x3(contraA.getBasis()) * ChaMatrixFromBtMat3x3(invfirstworldtra.getBasis()) * ChaMatrixFromBtMat3x3(curworldtra.getBasis()) * ChaMatrixFromBtMat3x3(contraA.getBasis().inverse());
							//ChaMatrix eulmat = ChaMatrixFromBtMat3x3(contraA.getBasis()) * ChaMatrixFromBtMat3x3(invfirstworldtra.getBasis()) * curlocalworldmat * ChaMatrixFromBtMat3x3(contraA.getBasis().inverse());
							ChaMatrix eulmat = TransZeroMat(ChaMatrixInv(firstlocalmat)) * curlocalmat;
							//ChaMatrix eulmat = ChaMatrixFromBtMat3x3(contraA.getBasis()) * curlocalworldmat * TransZeroMat(ChaMatrixInv(firstlocalmat)) * ChaMatrixFromBtMat3x3(contraA.getBasis().inverse());
							...
							CQuaternion newrotq;
							//newrotq = contrainvArot * currotz * curroty * currotx * contraArot * firstworldrot;
							//newrotq = contraArot * currotz * curroty * currotx * contrainvArot * firstworldrot;
							//newrotq = firstworldrot * contraArot * currotx * curroty * currotz * contrainvArot;
							//newrotq = contraArot * firstworldrot * currotx * curroty * currotz * contrainvArot;
							//newrotq = contraArot * currotz * curroty * currotx * firstworldrot * contrainvArot;
							//newrotq = contraArot * currotz * curroty * currotx * firstworldrot * contrainvArot;


							//rotz * roty * rotx部分はQの回転で表すと次の式
							//invA * cur * invfirst * A

							//newrotq = parrotq * contraArot * parrotq * currotz * curroty * currotx * firstworldrot * contrainvArot;
							//newrotq = contraArot * currotz * curroty * currotx * firstworldrot * contrainvArot;
							//newrotq = currotz * curroty * currotx * firstworldrot;
							//newrotq = firstlocalq * currotz * curroty * currotx;
							//newrotq = currotz * curroty * currotx * firstlocalq;
							...
							//newrotq = currotz * curroty * currotx;
							//btTransform invcontraA = contraA.inverse();
							//ChaMatrix newmatx = ChaMatrixFromBtTransform(contraA.getBasis(), contraA.getOrigin()) * newrotq.MakeRotMatX() * firstworldmatx * ChaMatrixFromBtTransform(invcontraA.getBasis(), invcontraA.getOrigin());
							//btVector3 newpivot;
							//newpivot.setX(newmatx._41);
							//newpivot.setY(newmatx._42);
							//newpivot.setZ(newmatx._43);
							//
							//btMatrix3x3 btnewmat;
							//btnewmat.setIdentity();
							//
							//newworldtra.setOrigin(newpivot);


//
//							//CQuaternion newrotq;
//							//newrotq.MakeFromBtMat3x3(newworldtra.getBasis());
////// 新しい回転を求める　ここまで
//
//// 新しいbtmatを求める　ここから
							//ChaMatrix btdiffrotmat;
							//btdiffrotmat = ChaMatrixFromBtMat3x3(curworldtra.getBasis().inverse()) * ChaMatrixFromBtMat3x3(newworldtra.getBasis());
							////btdiffrotmat = ChaMatrixFromBtMat3x3(newworldtra.getBasis()) * ChaMatrixFromBtMat3x3(curworldtra.getBasis().inverse());
							//btdiffrotmat = TransZeroMat(btdiffrotmat);
							//CQuaternion btdiffrotq;
							//CMotionPoint btdiffmp;
							//btdiffmp.CalcQandTra(btdiffrotmat, parbone);
							//btdiffrotq = btdiffmp.GetQ();


							//ChaVector3 parpos2;
							//ChaVector3TransformCoord(&parpos2, &parbone->GetJointFPos(), &parbone->GetBtMat());//親の位置は動かないはず
							...
							ChaMatrixTranslation(&befrot, -rotcenter.x, -rotcenter.y, -rotcenter.z);
							ChaMatrixTranslation(&aftrot, rotcenter.x, rotcenter.y, rotcenter.z);
							//ChaMatrix rotmat = befrot * btdiffrotq.MakeRotMatX() * aftrot;
							//ChaMatrix rotmat = btdiffrotq.MakeRotMatX();
							...
							//curroty.SetAxisAndRot(ChaVector3(0.0f, 1.0f, 0.0f), euly);
							//currotz.SetAxisAndRot(ChaVector3(0.0f, 0.0f, 1.0f), eulz);
							//CQuaternion currotxyz;
							//currotxyz = currotz * curroty * currotx;
							//ChaMatrix transmat = ChaMatrixFromBtMat3x3(contraA.getBasis().inverse()) * currotxyz.MakeRotMatX() * ChaMatrixFromBtMat3x3(contraA.getBasis());
...
////　角度制限をする　ここから
							//btMatrix3x3 chkdiffmat = firstworldmat.inverse() * newworldtra.getBasis();
							//btMatrix3x3 chkdiffmat = newworldtra.getBasis() * firstworldmat.inverse();
							//btMatrix3x3 chkeulmat = contraA.getBasis().inverse() * chkdiffmat * contraA.getBasis();
							//btMatrix3x3 chkeulmat = contraA.getBasis() * chkdiffmat * contraA.getBasis().inverse();
							//ChaMatrix chkdiffmat = ChaMatrixFromBtMat3x3(invfirstworldtra.getBasis()) * ChaMatrixFromBtMat3x3(newworldtra.getBasis());
							//ChaMatrix chkeulmat = ChaMatrixFromBtMat3x3(contraA.getBasis()) * chkdiffmat * ChaMatrixFromBtMat3x3(contraA.getBasis().inverse());
							//ChaMatrix chkeulmat = ChaMatrixFromBtMat3x3(contraA.getBasis()) * ChaMatrixFromBtMat3x3(invfirstworldtra.getBasis()) * ChaMatrixFromBtMat3x3(newworldtra.getBasis()) * ChaMatrixFromBtMat3x3(contraA.getBasis().inverse());
							//ChaMatrix chkeulmat = ChaMatrixFromBtMat3x3(contraA.getBasis()) * ChaMatrixFromBtMat3x3(invfirstworldtra.getBasis()) * newlocalworldrotmat * ChaMatrixFromBtMat3x3(contraA.getBasis().inverse());
							ChaMatrix chkeulmat = TransZeroMat(ChaMatrixInv(firstlocalmat)) * newlocalrotmat;



							//btScalar chkeulz = 0.0;
							//btScalar chkeuly = 0.0;
							//btScalar chkeulx = 0.0;
							ChaVector3 chkbefeul = ChaVector3(0.0f, 0.0f, 0.0f);
							ChaVector3 chkeul = ChaVector3(0.0f, 0.0f, 0.0f);
							//worldtra.getBasis().getEulerZYX(eulz, euly, eulx, 1);
							//chkeulmat.getEulerZYX(chkeulz, chkeuly, chkeulx, 1);
