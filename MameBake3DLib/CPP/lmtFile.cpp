...
int CLmtFile::WriteLmtFile( WCHAR* strpath, CModel* srcmodel, char* fbxcomment )
{
	...
	//WriteLmtReq(g_limitdegflag, m_model->GetTopBone(false));//g_limitdegflag‚Ílimitangle‚Ìchk’l—p
...
int CLmtFile::LoadLmtFile( WCHAR* strpath, CModel* srcmodel, char* fbxcomment )
{
	...
	//FileCommentƒ^ƒO‚ª‚ ‚éê‡‚É‚Ífbxcomment‚Æ”äŠrƒ`ƒFƒbƒN
	...
	while( findflag ){
		...
		if( ret == 0 ){
			CallF( ReadBone(g_limitdegflag, &bonebuf ), return 1 );//g_limitdegflag‚Ílimitangle‚Ìchk’l—p
		}else{
			findflag = 0;
		}
	}
...
int CLmtFile::ReadBone(bool limitdegflag, XMLIOBUF* xmliobuf)
{
	...
	***/
	...
	//if ((tmpint >= -180) && (tmpint <= 180)){//2022/12/18 comment out : ƒIƒCƒ‰[ŠpƒI[ƒo[‚P‚W‚O‘Î‰ž‚Ì‚½‚ß
