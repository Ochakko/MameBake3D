...
int CBone::CopyWorldToLimitedWorld(int srcmotid, double srcframe)//制限角度無しの姿勢を制限有りの姿勢にコピーする
{
	...
	if (curmp) {
		...
		//bool directsetflag = false;
		bool directsetflag = true;//2023/02/08 copyなのでtrue. 角度制限は後で別関数でする.
		bool infooutflag = false;
		int setchildflag = 1;
		SetWorldMat(limitdegflag, directsetflag, infooutflag, setchildflag, srcmotid, roundingframe, newwm);
	}
	...
	int srcmotid, double srcframe, ChaMatrix srcmat, int onlycheck)
{
	...
	ChaVector3 saveeul;
	saveworldmat = GetWorldMat(limitdegflag, srcmotid, roundingframe, curmp);//!!!!!!!!! 変更前を保存
...
int CBone::PasteMotionPoint(bool limitdegflag, int srcmotid, double srcframe, CMotionPoint srcmp)
{
	...
	if (newmp){
		...
		if (parentbone){
			...
			if (parmp){
				setmat = setmat * parentbone->GetWorldMat(limitdegflag, srcmotid, roundingframe, parmp);//copy情報はローカルなのでグロバールにする
			}
		}
		...
		////##################################################################################
		////2023/02/08 グローバル変数で挟んでの機能選択はやめて　引数による機能選択にするべき
		////後でメンテ予定
		////##################################################################################
		...
		//setmatflag == 1の場合には
		//RotBoneQReq内部から　directsetflag = trueで　SetWorldMatを呼び出す 
		//よってここでの　g_underRetargetFlag操作は不要
