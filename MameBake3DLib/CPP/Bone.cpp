	...
	int srcmotid, double srcframe, ChaMatrix srcmat, int onlycheck)
{
	...
	//2023/02/08
	//g_underRetargetFlagオンオフでの機能選択は　まだ廃止できていない
	//RotBoneQReq-->SetWorldMat-->RotBoneQReq-->SetWorldMat-->...のような呼び出しにおいて
	//持続的に　SetWorldMatのdirectsetflagを指定することが出来ていないから
	//後のバージョンの課題
	//ver1.2.0.10RC11において　(directsetflag == false)の判定だけでは
	//コピーペーストのペーストのときに不具合が出た
	//しょうがなく　g_underRetargetFlagオンオフによる機能選択を部分的に復活
	if ((directsetflag == false) && (g_underRetargetFlag == false)) {
