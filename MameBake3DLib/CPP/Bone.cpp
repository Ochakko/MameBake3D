...
float CBone::CalcAxisMatX_Manipulator(bool limitdegflag, int srcboneaxis, int bindflag, CBone* childbone, ChaMatrix* dstmat, int setstartflag)
{
	...
	if (GetParent(false)) {
		if (GetParent(false)->IsSkeleton()) {
			...
			tmpparentzerofm = GetParent(false)->GetENullMatrix();
			tmpparentlimwm = GetParent(false)->GetENullMatrix();
			tmpparentbtmat = GetParent(false)->GetENullMatrix();
		}
		else {
	...
	if (directsetflag == false) {
		...
		if (onlycheck == 0) {
			...
			if (setchildflag == 1) {
				if (GetChild(false)) {
					bool setbroflag = true;
					GetChild(false)->UpdateParentWMReq(limitdegflag, setbroflag, srcmotid, roundingframe,
						saveworldmat, srcmat);
				}
			}
		}
	...
	int srcmotid, double srcframe, CMotionPoint* srcmp, ChaVector3* dsteul)//default : dsteul = 0
{
	...
	if (IsNull()) {
		...
		return curmat;//!!!!!!!!!!!!  NormalでもNullでも無い場合　identityを返す
	}
