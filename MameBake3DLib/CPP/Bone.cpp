...
CMotionPoint* CBone::RotBoneQReq(bool infooutflag, CMotionPoint* parmp, int srcmotid, double srcframe, CQuaternion rotq, CBone* bvhbone, ChaVector3 traanim, int setmatflag, ChaMatrix* psetmat)
{
	...
	if (m_child && curmp){
		m_child->RotBoneQReq(infooutflag, curmp, srcmotid, srcframe, rotq);
	}
	if (m_brother && parmp){
		m_brother->RotBoneQReq(infooutflag, parmp, srcmotid, srcframe, rotq);
	}
	return curmp;
}
...
int CBone::SetWorldMatFromEulAndScale(int inittraflag, int setchildflag, ChaVector3 srceul, ChaVector3 srcscale, int srcmotid, double srcframe)
{
	//anglelimitをした後のオイラー角が渡される。anglelimitはCBone::SetWorldMatで処理する。
	if (!m_child) {
		return 0;
	...
	newlocalmat = befrotmat * newscalemat * newrotmat * aftrotmat;//!!!!!!!!!!!!!! new scale
