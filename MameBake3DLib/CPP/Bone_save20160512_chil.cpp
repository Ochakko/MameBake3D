...
int CBone::CalcAxisMatX()
{
	...
	if (m_child){
		ChaVector3TransformCoord(&curpos, &(GetJointFPos()), &m_startmat2);
		ChaVector3TransformCoord(&chilpos, &(m_child->GetJointFPos()), &(m_child->m_startmat2));
...
int CBone::CalcAxisMat( int firstflag, float delta )
{
	...
	if( delta != 0.0f ){
		...
		if (m_child){
			ChaVector3TransformCoord(&gparpos, &(GetJointFPos()), &m_startmat2);
			ChaVector3TransformCoord(&gchilpos, &(m_child->GetJointFPos()), &(m_child->m_startmat2));
...
void CBone::CalcFirstAxisMatX()
{
	...
	if (m_child){
		/*
		ChaVector3TransformCoord(&curpos, &(GetJointFPos()), &m_firstmat);
		ChaVector3TransformCoord(&chilpos, &(m_child->GetJointFPos()), &(m_child->m_firstmat));

		CalcAxisMatX_aft(curpos, chilpos, &m_firstaxismatX);
		*/
		CalcAxisMatX_aft(GetJointFPos(), m_child->GetJointFPos(), &m_firstaxismatX);
	}
