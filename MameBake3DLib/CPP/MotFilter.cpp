...
void CMotFilter::FilterReq(CModel* srcmodel, CBone* curbone, int srcmotid, int srcstartframe, int srcendframe)
{
	...
	if (curbone){
		...
		for (frame = srcstartframe; frame <= srcendframe; frame++){
			...
			ChaVector3 cureul = curbone->CalcLocalEulXYZ(-1, srcmotid, (double)frame, BEFEUL_DIRECT, 0, &befeul);// axiskind = -1 --> m_anglelimitの座標系
		...
		for (frame = srcstartframe; frame <= srcendframe; frame++){
			curbone->SetWorldMatFromEulAndTra(1, m_smootheul[frame - srcstartframe], m_smoothtra[frame - srcstartframe], srcmotid, (double)frame);
