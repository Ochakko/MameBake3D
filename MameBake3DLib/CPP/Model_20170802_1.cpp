				...
				if (boneptr){
					...
					while (chilbone){
						...
						if (renderflag == 1){
							...
							ChaVector3TransformCoord(&aftchilpos, &chilbone->GetJointFPos(), &(boneptr->GetCurMp().GetWorldMat()));
...
int CModel::GZeroRot(CEditRange* erptr, int srcboneno, ChaVector3 targetpos, int maxlevel)
{
		...
		while (childbone){
			...
			if (childbone->GetJointFPos() != parbone->GetJointFPos()){
				...
				if (fabs(rotrad2) > 1.0e-4){
					...
					ChaVector3TransformCoord(&newparpos, &jointfpos, &newbtmat);
					jointfpos = childbone->GetJointFPos();
					ChaVector3TransformCoord(&newchilpos, &jointfpos, &newbtmat);
...
int CModel::GZeroMV(CEditRange* erptr, int srcboneno, ChaVector3 diffvec)
{
	...
	ChaVector3 mvvec = diffvec * g_gzeromvrate;
	...
	//while (curbone && (curbone->GetPosConstraint() == 0)){
		...
		if (parbone){
			...
			while (childbone){
				...
				jointfpos = parbone->GetJointFPos();
				ChaVector3TransformCoord(&newparpos, &jointfpos, &newbtmat);
				//ChaMatrix befbtmat = parbone->GetBtMat();
				//ChaVector3TransformCoord(&newparpos, &jointfpos, &befbtmat);
...
int CModel::ImpulseBoneRagdoll(int onlyoneflag, CEditRange* erptr, int srcboneno, ChaVector3 addtra)
{
	...
	ChaVector3 impulse = addtra * g_gzeromvrate;
