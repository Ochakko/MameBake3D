...
ChaMatrix ChaMatrixKeepScale(ChaMatrix srcmat, ChaVector3 srcsvec)
{
	...
	if (((srcsvec.x - 1.0) > -0.00003) && ((srcsvec.x - 1.0) < 0.00003)) {
		srcsvec.x = 1.0;//誤差で急に大きさが変わるのを防止
	}
	if (srcsvec.x < 0.00003) {
		srcsvec.x = 0.00003;//0scale禁止
	}
	if (((srcsvec.y - 1.0) > -0.00003) && ((srcsvec.y - 1.0) < 0.00003)) {
		srcsvec.y = 1.0;//誤差で急に大きさが変わるのを防止
	}
	if (srcsvec.y < 0.00003) {
		srcsvec.y = 0.00003;//0scale禁止
	}
	if (((srcsvec.z - 1.0) > -0.00003) && ((srcsvec.z - 1.0) < 0.00003)) {
		srcsvec.z = 1.0;//誤差で急に大きさが変わるのを防止
	}
	if (srcsvec.z < 0.00003) {
		srcsvec.z = 0.00003;//0scale禁止
	}
