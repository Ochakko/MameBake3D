...
int CPluginElem::CreateMotionBrush(double srcstartframe, double srcendframe, double srcapplyframe, double srcframeleng, float* dstvalue)
{
	int ret = 0;
	...
	if( (validflag == 1) && MBCreateMotionBrush){//！！！！！！！ validflagに注意！！！！！！
		...
		if ((srcstartframe >= 0.0) && (srcstartframe < 1e5) && (srcendframe >= srcstartframe) && (srcendframe < 1e5) &&
			...
			ret = MBCreateMotionBrush(srcstartframe, srcendframe, srcapplyframe, srcframeleng, dstvalue);
