...
int CUndoSprite::CreateSprites(ID3D11Device* pdev, ID3D11DeviceContext* pd3dImmediateContext, WCHAR* mpath)
{
	...
	_ASSERT(m_sprite[0]);
	CallF(m_sprite[0]->Create(pd3dImmediateContext, mpath, L"0.png", 0, 0), return 1);

	m_sprite[1] = new CMySprite(pdev);
	_ASSERT(m_sprite[1]);
	CallF(m_sprite[1]->Create(pd3dImmediateContext, mpath, L"1.png", 0, 0), return 1);

	m_sprite[2] = new CMySprite(pdev);
	_ASSERT(m_sprite[2]);
	CallF(m_sprite[2]->Create(pd3dImmediateContext, mpath, L"2.png", 0, 0), return 1);

	m_sprite[3] = new CMySprite(pdev);
	_ASSERT(m_sprite[3]);
	CallF(m_sprite[3]->Create(pd3dImmediateContext, mpath, L"3.png", 0, 0), return 1);

	m_sprite[4] = new CMySprite(pdev);
	_ASSERT(m_sprite[4]);
	CallF(m_sprite[4]->Create(pd3dImmediateContext, mpath, L"4.png", 0, 0), return 1);

	m_sprite[5] = new CMySprite(pdev);
	_ASSERT(m_sprite[5]);
	CallF(m_sprite[5]->Create(pd3dImmediateContext, mpath, L"5.png", 0, 0), return 1);

	m_sprite[6] = new CMySprite(pdev);
	_ASSERT(m_sprite[6]);
	CallF(m_sprite[6]->Create(pd3dImmediateContext, mpath, L"6.png", 0, 0), return 1);

	m_sprite[7] = new CMySprite(pdev);
	_ASSERT(m_sprite[7]);
	CallF(m_sprite[7]->Create(pd3dImmediateContext, mpath, L"7.png", 0, 0), return 1);

	m_sprite[8] = new CMySprite(pdev);
	_ASSERT(m_sprite[8]);
	CallF(m_sprite[8]->Create(pd3dImmediateContext, mpath, L"8.png", 0, 0), return 1);

	m_sprite[9] = new CMySprite(pdev);
	_ASSERT(m_sprite[9]);
	CallF(m_sprite[9]->Create(pd3dImmediateContext, mpath, L"9.png", 0, 0), return 1);

	m_sprite[10] = new CMySprite(pdev);
	_ASSERT(m_sprite[10]);
	CallF(m_sprite[10]->Create(pd3dImmediateContext, mpath, L"R_32x32.png", 0, 0), return 1);

	m_sprite[11] = new CMySprite(pdev);
	_ASSERT(m_sprite[11]);
	CallF(m_sprite[11]->Create(pd3dImmediateContext, mpath, L"W_32x32.png", 0, 0), return 1);
...
int CUndoSprite::Render(ID3D11DeviceContext* pd3dImmediateContext, int undoR, int undoW)
{
