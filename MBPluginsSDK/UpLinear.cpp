	...
	if ((srcstartframe >= 0.0) && (srcstartframe < 1e5) && (srcendframe >= srcstartframe) && (srcendframe < 1e5) &&
		...
		if (frameT >= 3) {
			...
			for (repeatscnt = 0; repeatscnt < repeats; repeatscnt++) {
				...
				startframe = (int)srcstartframe + repeatscnt * frameT;
				endframe = startframe + frameT;
				applyframe = startframe + (int)(srcapplyframe - srcstartframe);
				if (srcmirroru) {
					if ((repeatscnt % 2) == 0) {
						invu = false;
					}
					...
					if ((framecnt >= (int)startframe) && (framecnt <= endframe)) {
						...
						if ((framecnt == startframe) && (framecnt != applyframe)) {
							...
							if (repeatscnt == 0) {
								...
								curscale = (float)(((double)framecnt - startframe) / halfcnt1);
							}
							else {
								if (invu) {
									if ((repeatscnt % 2) == 0) {
							...
							if (repeatscnt == (repeats - 1)) {
								if (invu) {
									if ((repeatscnt % 2) == 0) {
										...
										curscale = 1.0f - (float)(((double)endframe - framecnt) / halfcnt2);
